...
#   psxrecomp docs/MULTI_DISC.md and does not exist yet. A set where every disc
#   boots the same program scaffolds that one program and records each disc's
#   identity in disc_probe.N.json + disc_set.json.
...
# (Project Studio). An array cannot survive `powershell -File`: the caller's
# argument quoting collapses a comma-joined list with spaces in it into one
# element. "|" is forbidden in Windows paths, so splitting on it is
...
if ($DiscCount -gt 1) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Remove-Item -Recurse -Force $DiscSetTmp -ErrorAction SilentlyContinue
        Write-Host ""
        throw "Nothing was created. See psxrecomp docs/MULTI_DISC.md for what this needs."
    }
}
...
$raidPng = Join-Path $TemplateDir "raid-discord.png"
if (Test-Path $raidPng) {
    Copy-Item -Force $raidPng (Join-Path $Root ".github\raid-discord.png")
}
