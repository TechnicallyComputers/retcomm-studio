<#
.SYNOPSIS
New n64lle per-game project scaffolding -- the Windows entry point.
...
runs the same script with the same arguments, so a fix to the flow reaches every
platform at once. Every question is asked in this console -- bash reads the
answers through the console's /dev/tty, exactly as it does on Linux.

Requires Git for Windows (for bash.exe), Python 3 and CMake on PATH. WSL and Git
Bash users can run setup_project.sh directly instead.

.PARAMETER Rom
Your legally-owned dump (.z64/.v64/.n64). Probed for identity; linked, never
copied, into the repository. Optional on a terminal: with no ROM given it is the
first question.

.PARAMETER Dir
Parent directory for the new repository (default: the parent of this n64lle
checkout, so the project lands beside your other repos).

.PARAMETER Name
Display name. Default: derived from the dump filename, or the cartridge header
when the filename gives nothing to work with.

.PARAMETER Players
Controller ports to declare, 1-4. This is what the launcher shows as its player
count.

.PARAMETER Yes
Non-interactive: every question takes its default; the build and network
toggles stay off unless flagged.

.PARAMETER Passthrough
Every other setup_project.sh flag passes straight through: --generate,
--no-git, --gh, --gh-owner <owner>, --public, --frames <n>, --step-cap <n>,
--copy-rom, --slug, --project, --exe. Run with -Help for the list.

.EXAMPLE
powershell -File tools\new_project\setup_project.ps1 -Rom C:\roms\game.z64 -Dir C:\src

.EXAMPLE
powershell -File tools\new_project\setup_project.ps1 -Rom game.z64 -Name "Mario Kart 64" -Players 4 -Yes --generate
#>
